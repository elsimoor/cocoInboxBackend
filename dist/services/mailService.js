...
class MailService {
    constructor() {
        ...
        if (mailchimpApiKey && mailchimpServerPrefix) {
            this.mailchimpClient = (0, mailchimp_transactional_1.default)(mailchimpApiKey);
        }
    ...
    async sendEmail(user, message) {
        ...
        if (smtpDevApiKey && smtpDevAccountId && smtpDevMailboxId) {
            const response = await axios_1.default.post(`https://api.smtp.dev/accounts/${smtpDevAccountId}/mailboxes/${smtpDevMailboxId}/messages`, {
                to,
                from: fromEmail,
                subject,
                text: text || '',
        ...
        throw new Error('All free tier domains have reached their hourly send limit. Please wait until the next hour or upgrade to the premium plan.');
    }
    /**
    ...
    async receiveEmails(user) {
        ...
        if (mailUser && mailPass && mailHost && mailPort) {
            try {
                ...
                const rawMessages = await Promise.all(results.map(async (item) => {
                ...
                try {
                    ...
                    const userEmails = await EphemeralEmail_1.default.find({ user_id: user.id, is_active: true }).lean();
                    const activeAddresses = userEmails.map((e) => (e.email_address || '').toLowerCase());
                    if (activeAddresses.length === 0) {
                        return [];
                    }
                    const filtered = rawMessages.filter((m) => {
                        const toField = (m.to || '').toLowerCase();
                        return activeAddresses.some((addr) => toField.includes(addr));
                    });
                    return filtered;
                }
                catch (filterErr) {
        ...
        const smtpDevMailboxId = process.env.SMTPDEV_MAILBOX_ID;
        if (smtpDevApiKey && smtpDevAccountId && smtpDevMailboxId) {
            const response = await axios_1.default.get(`https://api.smtp.dev/accounts/${smtpDevAccountId}/mailboxes/${smtpDevMailboxId}/messages`, {
                headers: {
                    'X-API-KEY': smtpDevApiKey,
