...
router.post('/checkout', authenticate, async (req: AuthRequest, res) => {
  if (!isStripeConfigured()) return res.status(501).json({ error: 'Billing not configured' })
  try {
    if (!req.user?.id) return res.status(401).json({ error: 'Unauthorized' })
  const stripe = new Stripe(process.env.STRIPE_SECRET_KEY as string, { apiVersion: '2022-11-15' })
  const priceId = process.env.STRIPE_PRICE_ID as string | undefined
  ...
  const stripe = new Stripe(process.env.STRIPE_SECRET_KEY as string, { apiVersion: '2022-11-15' })
    ...
    if (!configurationId) {
      return res.status(501).json({
        error:
          'Stripe Billing Portal is not configured for this mode. Create a default configuration in test mode at https://dashboard.stripe.com/test/settings/billing/portal or set STRIPE_PORTAL_CONFIGURATION_ID.',
      })
    }
    ...
    if (e && typeof e.message === 'string' && e.message.includes('No configuration provided')) {
      return res.status(501).json({
        error:
          'Stripe Billing Portal not configured. Create your default portal configuration in test mode: https://dashboard.stripe.com/test/settings/billing/portal',
      })
    }
...
router.get('/confirm', async (req, res) => {
  ...
  try {
    ...
    if (!customerId) return res.status(400).json({ error: 'No customer associated' })

    const user = await User.findOne({ stripeCustomerId: customerId })
    if (!user) return res.status(404).json({ error: 'User not found for session' })
    user.is_pro = true
